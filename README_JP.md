...
インストール後、環境設定 (⌘,) を開いて、編集モード、テーマ、フォントなど、MiaoYan の豊富なカスタマイズオプションを確認し、最適な執筆環境を構築することをお勧めします。

## 分割編集プレビューモード

編集とプレビューを並べて表示し、60fps のリアルタイムプレビューと双方向スクロール同期を実現。

**クイック切り替え**：`⌘\` を押すと分割ビューモードを即座に切り替えられます。または設定 → インターフェース → 編集モード → 分割モードで有効化できます。
